...
function CarCard({
  ...
  return (
    <article
      id={`car-${car.id}`}
      className="group bg-white overflow-hidden flex flex-col shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-gray-100"
    ...
    >
      ...
      <div className="relative bg-gradient-to-br from-slate-50 to-slate-100 h-52 flex items-center justify-center overflow-hidden">
        {category && (
          <span
            className="absolute top-4 left-4 z-10 text-white text-[11px] font-bold uppercase tracking-wider px-3 py-1"
            style={{
              backgroundColor: "var(--t-badge-bg, var(--t-primary))",
              borderRadius: "calc(var(--t-radius, 1rem) * 0.6)",
            }}
          >
            {category}
          </span>
        )}
        {/* Premium gloss sweep effect on car image */}
        <div className="relative w-full h-64 px-4 premium-tilt">
          <Image
            src={car.image}
            alt={car.name}
            fill
            className="object-contain drop-shadow-md inner-image"
      ...
      <div className="flex flex-col flex-1 p-6">
        <h3 className="font-bold text-[20px] mb-3" style={{ color: "var(--t-primary)" }}>
          {car.name}
        </h3>
        ...
        <div className="flex items-end justify-between mt-auto">
          <div>
            <div className="flex items-baseline gap-1">
              ...
              <span className="text-[24px] font-extrabold leading-none" style={{ color: "var(--t-primary)" }}>
                {priceFormatted}
              </span>
            </div>
          ...
          <a
            ...
            className="text-white font-semibold text-[13px] px-5 py-2.5 transition-all duration-200 hover:shadow-lg hover:brightness-90 active:brightness-75"
            style={{
...
function CTACard({ phone }: { phone: string }) {
  return (
    <div
      className="group overflow-hidden flex flex-col shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border border-white/10 p-6 text-white justify-between min-h-[380px] h-full relative"
      style={{
    ...
    >
      ...
      <div
        className="absolute -right-8 -top-8 w-32 h-32 rounded-full blur-xl group-hover:scale-125 transition-transform duration-500"
        style={{ backgroundColor: "color-mix(in srgb, var(--t-accent) 15%, transparent)" }}
      />

      <div className="flex flex-col items-center justify-center text-center flex-1 py-8 z-10">
        <div
          className="w-16 h-16 border flex items-center justify-center mb-6 shrink-0"
          style={{
            borderRadius: "calc(var(--t-radius, 1rem) * 0.8)",
        ...
        <h3 className="text-xl font-bold text-white mb-2">Ingin Unit Lain?</h3>
      ...
      <a
        ...
        target="_blank"
        rel="noopener noreferrer"
        className="w-full py-3.5 font-extrabold text-[13px] text-center transition-all flex items-center justify-center gap-2 z-10 shrink-0 shadow-md hover:brightness-110 active:brightness-90"
        style={{
